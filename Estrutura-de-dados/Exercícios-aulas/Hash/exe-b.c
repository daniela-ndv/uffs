/**
 * Implementação com tratamento de conflitos na tabela Hash
*/
...
void insereNaTabela(Estudante* tabela[], Estudante* e){
    ...
    if(tabela[indice] == NULL){
        ...
        // Insere o novo elemento no início da lista (em tabela[indice])
        e->next = tabela[indice];
        tabela[indice] = e;    
        printf("Inseriu %s na posição %d - Sem conflito\n", e->nome, indice);    
    }
}
...
Estudante* buscaPorMatricula(Estudante* tabela[], int matricula){
    ...
    for(Estudante* aux = tabela[indice]; aux != NULL; aux = aux->next){
...
int main(void){
    ...
    Estudante e1 = {19, "Fulano", "CC", NULL};
    Estudante e2 = {15, "Ciclano", "CC", NULL};
    Estudante e3 = {25, "Beltrano", "CC", NULL};
    Estudante e4 = {17, "Maria", "CC", NULL};
    Estudante e5 = {8, "Dani", "CC", NULL};
